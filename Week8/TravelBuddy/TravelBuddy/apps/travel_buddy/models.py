...
class userManager(models.Manager):
    def validate (self, postData):
        ...
        if not Password_Regex.match(postData['password']):
            errors.append("Password needs to be between 8-15 characters")
        if postData['password'] != postData['confirm_password']:
            errors.append("Your passwords don't match")
        if len(postData['username']) < 3:
            errors.append("Username needs to be more than 3 characters")
        if len(postData['name']) < 3:
            errors.append("Name needs to be more than 3 characters")
    ...
    def login(self, postData):
        ...
        if 'username' in postData and 'password' in postData:
            ...
            except: #if the user doesnt exist from the .get(.get returns nothin, this 'except' prevents an error message)
